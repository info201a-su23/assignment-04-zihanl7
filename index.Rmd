...
```{r}
ggplot(data = data, aes(x = white_jail_pop_rate, y = black_jail_pop_rate)) +
  geom_point() +
...
data$region <- paste("washington,", data$county, sep = "")

wa_map <- map_data("county", region = "washington")

merged_data <- merge(wa_map, data, by.x = "region", by.y = "county", all.x = TRUE)
